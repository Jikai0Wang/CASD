...
def generate_candidates_and_draft_buffer(logits, input_ids, datastore, token_spans, top_p=0., temperature=1., max_num_draft=64, device="cuda"):
    ...
    input_ids_extend = torch.cat([input_ids.squeeze(0), candidates_logit.to(input_ids.device)], dim=-1)
    ...
    tree_candidates = candidates[draft_buffers["tree_indices"].to(candidates.device)]
    ...
    cart_candidates = tree_candidates_ext[draft_buffers["retrieve_indices"].to(tree_candidates_ext.device)]
...
def evaluate_posterior(
    logits, candidates, eos_token_id, threshold=0.01, k=0
):
    """
    ...
    p_eos=p[:,:,eos_token_id].unsqueeze(2)
    threshold=torch.where(p_eos>threshold,p_eos,threshold)
    c=candidates[:, 1:]
    c_p=torch.gather(p,2,c.unsqueeze(2))
    posterior_mask = (c_p > threshold).int().squeeze(2)

    if k>0:
        topk_values, topk_indices = torch.topk(logits[:, :-1], k, dim=2)
        candidates_expanded = candidates[:, 1:].unsqueeze(-1)
        posterior_mask_topk = (candidates_expanded == topk_indices).any(dim=2).int()
        posterior_mask = posterior_mask * posterior_mask_topk
