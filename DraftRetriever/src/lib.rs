...
impl Drop for Writer {
    fn drop(
        ...
        self.finalize().unwrap_or_default();
    }
}
...
impl Reader {
    ...
    fn new(
        ...
        while bytes_read < index_file_len {
            ...
            sub_indexes.push(
                SubIndex {
                    data,
                    index_file: Cursor::new(fs::read(index_file_path).unwrap_or_default()), // BufReader::new(File::open(index_file_path)?), // Cursor::new(fs::read(index_file_path).unwrap_or_default()),
                    suffixes_file_start,
                    suffixes_file_end,
                }
            );
    ...
    fn search(
        ...
        let substring_i32: Vec<i32> = py_substring.iter().map(|item| item.extract().unwrap_or(0)).collect();

        let results = Arc::new(Mutex::new(Vec::new()));

        // Parallel iteration over sub_indexes
        self.sub_indexes.par_iter_mut().for_each(|sub_index| {
            let mut start_of_indices = None;
            let mut end_of_indices = None;

            // Binary search to find the start of matching suffixes
            ...
            while left_anchor <= right_anchor {
                let middle_anchor = left_anchor + ((right_anchor - left_anchor) / 8 * 4);
                if let Ok(_) = sub_index.index_file.seek(SeekFrom::Start(middle_anchor as u64)) {
                    if let Ok(data_index) = sub_index.index_file.read_i32::<LittleEndian>() {
                        let line = &sub_index.data[data_index as usize..];

                        if line.starts_with(&substring_i32) {
                            start_of_indices = Some(middle_anchor);
                            right_anchor = middle_anchor.saturating_sub(4);
                        } else {
                            match line.cmp(&substring_i32) {
                                std::cmp::Ordering::Less => left_anchor = middle_anchor + 4,
                                std::cmp::Ordering::Greater => right_anchor = middle_anchor.saturating_sub(4),
            ...
            // Binary search to find the end of matching suffixes
            left_anchor = start_of_indices.unwrap();
            right_anchor = sub_index.suffixes_file_end - 4;
            while left_anchor <= right_anchor {
                let middle_anchor = left_anchor + ((right_anchor - left_anchor) / 8 * 4);
                if let Ok(_) = sub_index.index_file.seek(SeekFrom::Start(middle_anchor as u64)) {
                    if let Ok(data_index) = sub_index.index_file.read_i32::<LittleEndian>() {
            ...
            if let (Some(start), Some(end)) = (start_of_indices, end_of_indices) {
                let mut suffixes = vec![0; (end - start + 4) as usize];

                if let Ok(_) = sub_index.index_file.seek(SeekFrom::Start(start as u64)) {
                    if let Ok(_) = sub_index.index_file.read_exact(&mut suffixes) {
                        ...
                        let initial_capacity = std::cmp::min(indices_size as usize, k as usize);
        ...
        for retrieved_token in &*results {
            for j in 0..retrieved_token.len() {
                ...
                let counter = cnt.entry(tmp_token.to_vec()).or_insert(0);
                *counter += 1;
            }
        }
        ...
        Ok((
            paths.into_iter().map(|path| pad_path(path, max_length, -2)).collect::<Vec<Vec<i32>>>(),
            draft_attn_mask,
            tree_indices,
            draft_position_ids,
            retrieve_indices,
...
fn cut_to_choices(paths: Vec<Vec<i32>>, choices: i32) -> Vec<Vec<i32>> {
    let mut count: Vec<(usize, usize)> = paths.iter()
                                               .map(|p| (p.iter().collect::<std::collections::HashSet<&i32>>().len(), paths.iter().position(|x| x == p).unwrap_or_default()))
                                               .collect();
...
fn get_draft_choices(paths: Vec<Vec<i32>>) -> (Vec<Vec<i32>>, i32) {
    ...
    let max_depth = paths.iter().map(|path| path.len() as i32).max().unwrap_or_default();
    ...
    let max_branch = path_dict.values().map(|v| v.len() as i32).max().unwrap_or_default();
...
fn generate_draft_buffers(draft_choices: Vec<Vec<i32>>, topk: i32) -> (Vec<Vec<i32>>, Vec<i32>, Vec<i32>, Vec<Vec<i32>>) {
    ...
    for i in 0..depth_counts.len() {
        for j in 0..depth_counts[i] {
            ...
            for c in 0..(cur_draft_choice.len() - 1) {
                let index = sorted_draft_choices.iter().position(|x| x[..=cmp::min(c, x.len() - 1)] == cur_draft_choice[..=cmp::min(c, cur_draft_choice.len() - 1)]).unwrap_or_default() + 1;
    ...
    for i in 0..sorted_draft_choices.len() {
        ...
        if retrieve_paths.contains(&cur_draft_choice) {
            ...
            for c in 0..cur_draft_choice.len() {
                let index = sorted_draft_choices.iter().position(|x| *x == cur_draft_choice[0..=c]).unwrap_or_default();
    ...
    let max_length = retrieve_indices_nest.iter().map(|x| x.len()).max().unwrap_or_default();
